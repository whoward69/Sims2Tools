...
namespace Sims2Tools.Cache
{
    public class DataCache
    {
        ...
        private static readonly string cacheSharedBasePath = $"{cacheBasePath}/Shared/.cache";
        ...
        public static readonly string CacheObjectsPath = $"{cacheSharedBasePath}/Objects";
        public static readonly string CacheClothesPath = $"{cacheSharedBasePath}/Clothes";
        ...
        public static readonly string MaxisObjectsFilename = "MaxisObjects";
        public static readonly string CustomObjectsFilename = "CustomObjects";
        ...
        static DataCache()
        {
            ...
            // I changed my mind as to where the clothes cache should live when Collection Manager came along!
            DirectoryInfo di = new DirectoryInfo($"{cacheFamilyManagerBasePath}/Clothes");
            if (di.Exists)
            {
                lock (di)
                {
                    // Get the list of files as we're about to move them
                    string[] cachePaths = Directory.GetFiles(di.FullName, "*.*", SearchOption.AllDirectories);

                    foreach (string fromPath in cachePaths)
                    {
                        string toPath = $"{cacheSharedBasePath}{fromPath.Substring(cacheFamilyManagerBasePath.Length)}";
        ...
        public static void DeleteFamilyManagerCaches()
        {
        ...
        public static void RecreateFamilyManagerCaches()
        {
            DeleteFamilyManagerCaches();
        ...
        private static void CreateObjectsCache()
        {
            if (!Directory.Exists(CacheObjectsPath))
            {
                Directory.CreateDirectory(CacheObjectsPath);
            }
        }
        ...
        public static void InvalidateHoodsCache()
        {
            if (Directory.Exists(CacheHoodsPath))
            {
        ...
        public static void InvalidateCareersCache(string type)
        {
            InvalidateCache($"{CacheCareersPath}/{type}.bin");
        }

        public static void InvalidateObjectsCache(string type)
        {
            InvalidateCache($"{CacheObjectsPath}/{type}.bin");
        }

        public static void InvalidateOutfitsCache(string type, TypeTypeID typeId)
        {
            if (typeId == Gzps.TYPE)
            {
                InvalidateCache($"{CacheClothesPath}/{type}.bin");
            }
            else
            {
                InvalidateCache($"{CacheJewelleryPath}/{type}.bin");
            }
        }

        private static void InvalidateCache(string cachePath)
        {
        ...
        internal static bool Serialize(Dictionary<TypeGUID, ObjectData> data, string cachePath, string cacheName)
        {
            try
            {
        ...
        internal static bool Deserialize(out Dictionary<TypeGUID, ObjectData> data, string cachePath, string cacheName)
        {
            try
            {
                using (FileStream fs = File.Open($"{cachePath}/{cacheName}.bin", FileMode.Open))
                {
                    data = (Dictionary<TypeGUID, ObjectData>)new BinaryFormatter().Deserialize(fs);
                }
            ...
            catch (Exception)
            {
                ...
                data = new Dictionary<TypeGUID, ObjectData>();
                return false;
            }
        }
